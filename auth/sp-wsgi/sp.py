...
class Cache(object):
    ...
    def get_user(self, environ, clear=False):
        ...
        if cookie:
            ...
            if morsel:
                try:
                    if clear:
                        return self.uid2user.pop(morsel.value)
...
class SSO(object):
    ...
    def redirect_to_auth(self, _cli, entity_id, came_from, sigalg=""):
        try:
            ...
            self.cache.relay_state.pop(_rstate)
            _sid = req_id
...
def main(environ, start_response, sp):
    ...
    body.append(
        "<p>Login Successful</p><br><pre>{authn_stmt}</pre>".format(
    ...
    resp = Response(body)
    return resp(environ, start_response)
    # logout(environ, start_response, sp)
...
def logout(environ, start_response, sp):
    user = CACHE.get_user(environ, clear=True)
            ...
            if binding == BINDING_HTTP_POST:
                ...
                for key, value in http_info["headers"]:
                    if key.lower() == "location":
                        ...
                        resp = Redirect(value) #, headers=[cookie])
                        return resp(environ, start_response)
